...
class ErrorNormalizationService:
    ...
    def normalize(
        ...
        category_hint = self._infer_category(summary, normalized_prefix)
        keywords = self._extract_keywords(f"{record.error_prefix} {summary}")
        taxonomy = classify_taxonomy(record.error_prefix, summary)
    ...
    def normalize_from_storage(self, raw_storage_reference: str) -> tuple[ProcessedErrorRecord, str]:
